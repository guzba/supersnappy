...
block varint:
  doAssert varint(0) == @[0.uint8]
  doAssert varint(64) == @[0x40.uint8]
  doAssert varint(2097150) == @[0xFE.uint8, 0xFF, 0x7F]
  doAssert varint(high(uint32)) == @[0xFF.uint8, 0xFF, 0xFF, 0xFF, 0x0F]
  doAssert varint(0xFF.uint8) == @[0xFF.uint8, 0x01]

  doAssert varint(varint(0)) == (0.uint32, 1)
  doAssert varint(varint(64)) == (64.uint32, 1)
  doAssert varint(varint(2097150)) == (2097150.uint32, 3)
  doAssert varint(varint(high(uint32))) == (high(uint32), 5)
